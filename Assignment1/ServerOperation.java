...
public class ServerOperation extends UnicastRemoteObject implements ProcessInterface,Runnable{
	...
	/*protected ServerOperation(Message message) throws RemoteException {
		this.message=message;
		// TODO Auto-generated constructor stub
	}*/

	private static ProcessInterface look_up;
	private int myProcessId=2;
	private int[] myVectorClock= {0,0,0};
	...
	public int[] getClock() {
        ...
        int[] clock = {0,0,0};
        try {
            if ( !new File("d:\\myVectorClock2.txt").exists())
                return clock;
            else {
                BufferedReader br = new BufferedReader(new FileReader(new File("d:\\myVectorClock2.txt")));
    ...
    public void putClock(int[] clock) {
        try {
            BufferedWriter bw = new BufferedWriter(new FileWriter(new File("d:\\myVectorClock2.txt")));
