...
public class ClientOperation extends UnicastRemoteObject implements ProcessInterface,Runnable{
	...
	/*protected ClientOperation(Message message) throws RemoteException {
	...
	public static void main(String[] args) 
		...
		try {
			//myVectorClock=ProcessDataConstants.broadcastMsg(new Message(myProcessId, "BROADCAST BY "+myProcessId, myVectorClock), look_up);
		}
		catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void broadcastMsg()  {
		...
		ProcessDataConstants.setCount(count);
	...
	public int[] getClock() {
        ...
        int[] clock = {0,0,0};
        try {
            if ( !new File("d:\\myVectorClock.txt").exists())
                return clock;
            else {
                BufferedReader br = new BufferedReader(new FileReader(new File("d:\\myVectorClock.txt")));
    ...
    public void putClock(int[] clock) {
        try {
            BufferedWriter bw = new BufferedWriter(new FileWriter(new File("d:\\myVectorClock.txt")));
